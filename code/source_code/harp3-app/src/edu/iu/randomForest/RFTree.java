 ...
 */
public class RFTree {
	private int leafClassThreshold=100;
	private List<Integer> attributes;
	private int numberofAttributes;
	private RTNode root;
	private int skipSize = 3;
	...
	public void constructTree(DataFrame data){
		...
		if(!data.getValues().isEmpty()){
			...
			System.err.println("Calling generate Node.");
			generateNode(root,0);
			System.err.println("Node generated.");
	...
	private void generateNode(RTNode node, int lev){
		...
		System.err.println("level : "+lev);
		double minEntropy = Double.MAX_VALUE;
		if(!node.isLeafNode()){
			if(lev > 20 || checkIfLeaf(node)){
			...
			for(int attribute : attributes){
				...
				if(classLabelChangeIndexes.size() > 10){
					for(int i=0; i< classLabelChangeIndexes.size(); i+=skipSize){
						double currentEntropy =checkSplit(node, classLabelChangeIndexes.get(i), minEntropy, attribute);
			...
			//stem.err.println("Left node data size: "+leftChild.getData().getValues().size());
			if(leftChild.getData().getValues().size() < leafClassThreshold){
				...
				generateNode(node.getLeftChild(), lev+1);
			}
			
			//check if right child can be a leaf node
			RTNode rightChild = node.getRightChild();
			//System.err.println("Right node data size: "+leftChild.getData().getValues().size());
			if(rightChild.getData().getValues().size() < leafClassThreshold){
				...
				generateNode(node.getRightChild(), lev+1);
			}
	...
	private int getMajorityClass(List<int[]> data) {
		...
		for(int i=0; i<data.size(); i++){
			if(data.get(i)[numberofAttributes-1]==-1){
				class0Count++;
			}else{
				class1Count++;
			}
		}
		return class0Count > class1Count ? -1 : 1;
	}
	...
	public void printTree(RTNode node){
		if(node!=null){
			System.err.println("Tree node is leaf ?"+node.isLeafNode());
